...
public class DatabaseUnitTest {
    ...
    public void test_11_CSV() throws Exception {
        String data = "01.01\t20161101\tAuthor 1\tScripture 1\tScr 1\tTitle 1\tExegesis 1\tVerse Week 1\tScripture Week 1\tVerse Month 1\tScripture Month 1\n" +
                "02.02\t20161102\tAuthor 2\tScripture 2\tScr 2\tTitle 2\tExegesis 2\tVerse Week 1\tScripture Week 1\t\t\n" +
                "03.03\t20161103\tAuthor 3\tScripture 3\tScr 3\tTitle 3\tExegesis 3\t\t\tVerse Month 3\tScripture Month 3\n" +
                "04.04\t20161104\tAuthor 4\tScripture 4\tScr 4\tTitle 4\tExegesis 4\t\t\t\t";

        db.importCSV(TEST_CSV, new ByteArrayInputStream(data.getBytes()));
        Assert.assertTrue(db.isInstalled(TEST_CSV));

        Assert.assertEquals(4, db.getDay(new GregorianCalendar(2016, 11 - 1, 1).getTime()).getCount());
        Assert.assertEquals(3, db.getDay(new GregorianCalendar(2016, 11 - 1, 2).getTime()).getCount());
        Assert.assertEquals(2, db.getDay(new GregorianCalendar(2016, 11 - 1, 3).getTime()).getCount());
        Assert.assertEquals(2, db.getDay(new GregorianCalendar(2016, 11 - 1, 4).getTime()).getCount());
        Assert.assertEquals(4, db.getCalendar().getCount());
        Assert.assertEquals(4 + 3 + 2 + 2, db.getList().getCount());
        Assert.assertEquals(4, db.getSearch("Exegesis").getCount());
    ...
    public void test_12_XML() throws Exception {
        String data = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
                ...
                "<entry date=\"2017-01-01\" description=\"Name of the Day 1\">" +
                ...
                "</entry>" +
                "<entry date=\"2017-01-02\" description=\"Name of the Day 2\">" +
                "<exegesis sourceId=\"123\" source=\"Name of the Book\" sourceChapter=\"234\" sourceVerse=\"345\" title=\"Title of the Day\" subtitle=\"Subtitle of the Day\">Text of the Day</exegesis>" +
                "<verse_of_the_day source=\"Scripture of the Day\">Verse of the Day</verse_of_the_day>" +
                "<verse_of_the_week source=\"Scripture of the Week\">Verse of the Week</verse_of_the_week>" +
                "<verse_of_the_month source=\"Scripture of the Month\">Verse of the Month</verse_of_the_month>" +
        ...
        Assert.assertEquals(5, db.getDay(new GregorianCalendar(2017, 1 - 1, 1).getTime()).getCount());
        Assert.assertEquals(5, db.getDay(new GregorianCalendar(2017, 1 - 1, 2).getTime()).getCount());
